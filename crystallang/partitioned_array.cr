...
class PartitionedArray
  ...
  #accessor %i[range_arr rel_arr db_size data_arr partition_amount_and_offset db_path db_name partition_addition_amount]
  property :range_arr, :rel_arr, :db_size, :data_arr, :partition_amount_and_offset, :db_path, :db_name, :partition_addition_amount
  ...
  @db_name : String
  @db_path : String
#  @partition_amount_and_offset : BigInt | Int32
  @partition_addition_amount : BigInt | Int32
  @range_arr : Array(BigInt)
  @rel_arr : Array(BigInt)
  @db_size : BigInt | Int32
  @data_arr : Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
  # not Array(Hash(String, Array(Array(BigInt | Bool | Float64 | Int32 | String | Nil))))
  # is  Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
  @allocated : Bool
  @dynamically_allocates : Bool
  @partition_amount_and_offset : BigInt | Int32


  @data_arr_hash_adder : Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
  #@partiton_amount_and_offset : BigInt | Int32
  #@last_entry : BigInt
  # Initialize the PartitionedArray
  ...
  def initialize(partition_addition_amount = PARTITION_ADDITION_AMOUNT, dynamically_allocates = DYNAMICALLY_ALLOCATES, db_size = DB_SIZE, partition_amount_and_offset = (PARTITION_AMOUNT + OFFSET), db_path = DEFAULT_PATH, db_name = DB_NAME)
    ...
    @allocated = false
    @db_path = db_path
    @data_arr = [] of Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
    
    # not Array(Array(Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))))
    #want Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
    
    
    
    
    # of Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))
    # not Array(Array(Hash(String, Array(Array(BigInt | Bool | Float64 | Int32 | String | Nil)))))
    # wan Array(Hash(String, Array(Array(BigInt | Bool | Float64 | Int32 | String | Nil))))
    @range_arr = [] of BigInt # the range array which maintains the partition locations
    @rel_arr = [] of BigInt # a basic array from 1..n used in binary search
    @db_name = db_name
    @data_arr_hash_adder = {} of String => Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))
  ...
  def set_partition_subelement(id, partition_id, &block)
    ...
    if id <= (@data_arr.size - 1) && partition
      if block_given? && partition[id].instance_of?(Hash)
        ...
        @data_arr[@range_arr[partition_id].to_a[id]] = {} of String => String
  ...
  def add(&block)
    ...
    @data_arr.each_with_index do |element, element_index|
      ...
      if element == {} of String => String && block_given? # (if element is nill, no data is added because the partition is "offline")
  ...
  def allocate(db_size = @db_size, partition_amount_and_offset = @partition_amount_and_offset, override = false)
    if !@allocated || override
      ...
      # hash that goes into an array of hashes
      

      # an array of hashes
      # create a hash that will fit into the next @data_arr element
      @data_arr_hash_adder : Array = {} of String => BigInt | JSON::Any | String | Array | Hash | Nil | Int32 | Int64
      #Array(Hash(String, Array(Hash(String, BigInt | Bool | Float64 | Int32 | String | Nil))))
      @data_arr = (0..(db_size * (partition_amount_and_offset - x))).to_a.map { @data_arr_hash_adder } # create an array of hashes # for an array that fills to less than the max of @rel_arr

      data_arr_size = @data_arr.size
      #temp = data_arr_size.times.map{ |i| i }
      
      array = [] of BigInt
    #  puts "temp: #{temp}"
      0.upto(@data_arr.size-1).each do |i|
       array << BigInt.new(i)
      end
      @rel_arr = array #@data_arr.size.times.map { |i| i }  # for an array that fills to less than the max of @rel_arr
  ...
  def load_from_files!(db_folder=@db_folder)
    ...
    data_arr_set_partitions = [] of String
