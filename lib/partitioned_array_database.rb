...
class PartitionedArrayDatabase
  attr_accessor :database_folder_name, :manager
  ...
  def initialize(database_folder_name: DATABASE_FOLDER_NAME)
    ...
    @pad = FileContextManagedPartitionedArrayManager.new(db_path: "#{database_folder_name}/#{DB_NAME}", fcmpa_db_folder_name: "#{database_folder_name}/#{FCMPAM_DB_INDEX_NAME}")
  end
end
